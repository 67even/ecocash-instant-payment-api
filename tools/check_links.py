...
trap this script exists to avoid:
  ...
  * Repository Markdown (README, CHANGELOG, reference/) uses relative
    links that are FILESYSTEM paths - `reference/ECOCASH-EIP-API.md` must exist
    on disk, because that is how GitHub renders them.

  * docs/ is a Jekyll site. Its links are PERMALINKS - `../api-reference/` resolves to
    /api-reference/, a URL that no file on disk matches. Checking those against the
...
def slug(heading):
    ...
    Runs of spaces are NOT collapsed - 'A - B' becomes 'a---b' - and a leading
    hyphen is KEPT: GitHub and kramdown's GFM parser both turn '☕ Java' into
    '-java', not 'java'."""
