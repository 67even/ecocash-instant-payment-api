...
SITE_URL = "67even.github.io/ecocash-instant-payment-api"
...
def pages():
    ...
    for f in sorted(os.listdir(DOCS)):
        ...
        # the card path is taken from the page's `image:` front matter, which
        # flattens nested permalinks (/reference/sdks-tab/ -> reference-sdks-tab)
        m = re.search(r"^image:\s*\n\s+path:\s*/assets/og/(\S+)\.png\s*$", fm, re.M)
        slug = m.group(1) if m else ("home" if permalink.strip("/") == "" else permalink.strip("/"))
...
<div class="pad">
  ...
  <div class="foot">
    ...
    <span class="kicker">cURL · PHP · Node.js · Python · Java · C#</span>
  </div>
</div>
"""
...
def build(check_only=False):
    ...
    with sync_playwright() as pw:
        ...
        for p in want:
            title = p["title"] if p["title"] != "Home" else "EcoCash EIP Developer Guide"
            ...
            os.unlink(tmp)
            print("  %-38s %-36s %6d bytes" % (p["file"], p["slug"] + ".png",
