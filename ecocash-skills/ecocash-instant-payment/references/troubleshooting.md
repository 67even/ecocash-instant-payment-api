<!-- GENERATED by tools/build_skill.py from README.md section 10 - edit the source, then re-run it. -->

# Troubleshooting

> Symptom -> cause -> fix, debugging tips, support contacts.
...
| Status is `null` in your code | Reading `transactionStatus` or `transactionOperationStatus` | Read `status` first ([8.7](code-examples.md#87-pitfalls-in-the-portals-sdk-samples)) |
...
| Refund rejected for `tranType` | The portal's pages disagree on `REF` vs `MER` | Try the other value ([4.5](first-payment.md#45-confirm-three-details-in-your-first-run)) |
